...
SCRIPT = Path(__file__).with_name("SPINE_GPEv7_PNADC_HISTORICAL_BACKCAST_HARDENING_v1.1.0.py")
spec = importlib.util.spec_from_file_location("hardening", SCRIPT)
assert spec and spec.loader
hardening = importlib.util.module_from_spec(spec)
sys.modules["hardening"] = hardening
...
def main() -> int:
    ...
    raw, model_summary = hardening.bootstrap_model_uncertainty(
        pooled,
        fh,
        hist.groupby(["source_period", *hardening.FEATURES], observed=True).agg(weight_total=("weight_total", "sum")).reset_index(),
        reps=2,
        n_splits=2,
        logger=hardening.setup_logger(False),
    )
    assert raw["replicate"].nunique() >= 1
    final = hardening.combine_uncertainty(estimates, model_summary)
