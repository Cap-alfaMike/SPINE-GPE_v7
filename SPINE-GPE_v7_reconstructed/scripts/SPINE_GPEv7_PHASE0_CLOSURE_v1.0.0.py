...
# -*- coding: utf-8 -*-
"""
SPINE-GPE v7 — Phase 0 Closure Engine v1.0.0
================================================
...
def default_codebook_path() -> Path:
    return Path(__file__).resolve().with_name("rais_editorial_codebook_ptbr_v1.0.0.csv")
...
def read_codebook(path: Path) -> pd.DataFrame:
    ...
    codebook["code_norm"] = codebook["code"].map(normalize_code)
...
def decode_dimension(frame: pd.DataFrame, dimension: str, codebook: pd.DataFrame) -> tuple[pd.Series, pd.Series, pd.Series]:
    codes = frame[dimension].map(normalize_code)
...
def run_rais_editorial(root: Path, dirs: dict[str, Path], run_id: str, active: pd.DataFrame, codebook_path: Path, logger: logging.Logger) -> dict[str, Any]:
    ...
    for dimension in CORE_EDITORIAL_DIMENSIONS:
        ...
        frame[code_col] = frame[dimension].map(normalize_code)
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    run_id = args.run_id or f"phase0_closure_{dt.datetime.now(dt.timezone.utc).strftime('%Y%m%dT%H%M%SZ')}_v100"
