...
# -*- coding: utf-8 -*-
"""
SPINE-GPE v7 — PNADc Direct Platform Certification Engine v1.1.0
=================================================================
...
Uso Colab:
  python SPINE_GPEv7_PNADC_CERTIFIER_v1.1.0.py \
...
def fetch_sidra_with_cache(
    session: requests.Session, table: int, key: str, output_dir: Path, logger: logging.Logger
    ...
    try:
        ...
        candidates = sorted(output_dir.glob(f"sidra_{table}_*.csv"), key=lambda x: x.stat().st_mtime, reverse=True)
        if candidates:
            logger.warning("SIDRA %s indisponível; usando snapshot %s", table, candidates[0])
            return pd.read_csv(candidates[0], dtype=str), "cache"
        raise RuntimeError(f"SIDRA ao vivo falhou e não há snapshot local: {live_exc}") from live_exc
...
def run_sidra_golden_tests(
    ...
    core_keys = {"platform_any", "platform_type", "income", "hours"}

    for key, table in tables.items():
        try:
            df, source = fetch_sidra_with_cache(session, table, key, output_dir, logger)
        except Exception as exc:
            logger.error("Falha SIDRA tabela %s: %s", table, exc)
            severity = "critical" if key in core_keys else "high"
        ...
        results.append(TestResult(
            ...
            evidence={
                "rows": len(df),
                "path": str(output_dir / f"sidra_{table}_{key}.csv"),
                "source": source,
            },
        ))
        ...
        if key == "platform_any":
            for year in (2022, 2024):
                total_candidates = sidra_candidates(
                    df, year,
                    includes=("trabalhou", "plataforma", "mil pessoas"),
                    excludes=("nao trabalhou", "percentual"),
                )
                official, ev = official_value_from_candidates(total_candidates)
                obs = estimate_lookup(estimates, year, "platform_any_direct", "total")
                results.append(compare_golden(
                    f"golden.{year}.platform_any_total", year,
                    obs.estimate if obs else None, official, 0.015,
                    {"table": table, **ev},
                ))

                pct_candidates = sidra_candidates(
                    df, year,
                    includes=("trabalhou", "plataforma", "percentual"),
                    excludes=("nao trabalhou", "mil pessoas"),
                )
                ...
                results.append(compare_golden(
                    f"golden.{year}.platform_any_percent", year,
                    obs_pct.estimate if obs_pct else None, official_pct, 0.02,
                    {"table": table, **pct_ev},
                ))
            ...
            for year in (2022, 2024):
                candidates = sidra_candidates(
                    df, year,
                    includes=("entrega", "mil pessoas"),
                    excludes=("percentual",),
                    exact_codes={"tipo de plataforma": "59658"},
                )
                official, ev = official_value_from_candidates(candidates)
                obs = estimate_lookup(estimates, year, "platform_delivery_direct", "total")
                results.append(compare_golden(
                    f"golden.{year}.platform_delivery_total", year,
                    obs.estimate if obs else None, official, 0.02,
                    {"table": table, **ev},
                ))
            ...
            for year in (2022, 2024):
                candidates = sidra_candidates(
                    df, year,
                    includes=("trabalhou", "plataforma", "reais"),
                    excludes=("nao trabalhou", "percentual"),
                )
                # Prefere rendimento médio total quando a tabela traz decomposições.
                if not candidates.empty:
                    text = candidates.apply(sidra_row_text, axis=1)
                    preferred = text.map(lambda x: "total" in x and "rendimento medio" in x)
                    if preferred.any():
                        candidates = candidates.loc[preferred]
                official, ev = official_value_from_candidates(candidates)
                obs = estimate_lookup(estimates, year, "platform_any_direct", "mean_monthly_income")
                results.append(compare_golden(
                    f"golden.{year}.platform_any_income", year,
                    obs.estimate if obs else None, official, 0.025,
                    {"table": table, **ev},
                ))
            ...
            for year in (2022, 2024):
                ...
                results.append(compare_golden(
                    f"golden.{year}.platform_any_informal_total", year,
                    obs.estimate if obs else None, official, 0.03,
                    {"table": table, **ev},
                ))
            ...
            for year in (2022, 2024):
                candidates = sidra_candidates(
                    df, year,
                    includes=("plataforma", "mil pessoas", "contribuicao"),
                    excludes=("nao contribuiu", "percentual"),
                )
                if not candidates.empty:
                    texts = candidates.apply(sidra_row_text, axis=1)
                    positive = texts.str.contains(r"contribuiu|contribuinte|\bsim\b", regex=True)
                    if positive.any():
                        candidates = candidates.loc[positive]
                official, ev = official_value_from_candidates(candidates)
                obs = estimate_lookup(estimates, year, "platform_any_social_security", "total")
                results.append(compare_golden(
                    f"golden.{year}.platform_any_social_security_total", year,
                    obs.estimate if obs else None, official, 0.03,
                    {"table": table, **ev},
                ))
