...
# -*- coding: utf-8 -*-
"""
SPINE-GPE v7 — PNADc Historical Backcast Publication Hardening & Uncertainty Engine v1.2.0
================================================================================
...
---
Auditoria leve:
  python SPINE_GPEv7_PNADC_HISTORICAL_BACKCAST_HARDENING_v1.2.0.py \
...
Hardening completo:
  python SPINE_GPEv7_PNADC_HISTORICAL_BACKCAST_HARDENING_v1.2.0.py \
    --root /content/drive/MyDrive/aCidadeAlgoritmica/SPINE-GPEv7 \
    --mode full --profile publication --bootstrap-reps 500 --strict
"""
...
VERSION = "1.2.0"
SCRIPT_NAME = "SPINE_GPEv7_PNADC_HISTORICAL_BACKCAST_HARDENING_v1.2.0.py"
...
def validate_run_id(value: str | None) -> str:
    if value is None:
        return make_run_id()
    candidate = str(value).strip()
    if not re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9_.-]{5,80}", candidate):
...
def validate_upstream(root: Path, tests: list[TestResult]) -> dict[str, Any]:
    ...
    freeze_path = root / "00_admin" / "PNADC_HISTORICAL_PROXY_CORE_FREEZE.json"
    freeze = None
    if freeze_path.exists():
        freeze = load_json(freeze_path)
        freeze_ok = str(freeze.get("status", "")).upper() in {"FROZEN", "FINAL_CERTIFIED", "CERTIFIED"}
        tests.append(TestResult(
            "upstream.historical.freeze", "PASS" if freeze_ok else "WARN", "high",
            "Freeze upstream localizado e válido." if freeze_ok else "Freeze upstream localizado, mas com status inesperado.",
            observed={"status": freeze.get("status"), "sha256": sha256_file(freeze_path)},
            expected="FROZEN/CERTIFIED",
        ))
    else:
        tests.append(TestResult(
            "upstream.historical.freeze", "WARN", "high",
            "Freeze upstream não localizado; o lock certificado permanece a âncora de execução.",
            observed=None, expected=str(freeze_path),
        ))
    return {
        "lock_path": str(lock_path),
        "lock_sha256": sha256_file(lock_path),
        "lock": lock,
        "freeze_path": str(freeze_path) if freeze_path.exists() else None,
...
def calibration_intercept_slope(y: np.ndarray, p: np.ndarray, w: np.ndarray) -> CalibrationFit:
    """Estima intercepto e slope de calibração com likelihood ponderada robusta.

    A implementação anterior usava BFGS sem gradiente e podia devolver NaN em
    eventos raros. Esta versão padroniza o logit, usa gradiente analítico,
    limites numéricos e fallback quase não penalizado.
    """
    ...
    if valid.sum() < 10:
        return CalibrationFit(None, None, "NOT_ESTIMABLE", "insufficient_valid_rows", 0)
    y, p, w = y[valid], p[valid], w[valid]
    if len(np.unique(y)) < 2:
        return CalibrationFit(None, None, "NOT_ESTIMABLE", "single_class", 0)

    epsilon = 1e-6
    n_clipped = int(((p <= epsilon) | (p >= 1 - epsilon)).sum())
    clipped = np.clip(p, epsilon, 1 - epsilon)
    x = logit(clipped)
    weights = normalize_sample_weights(w)
    x_mean = float(np.average(x, weights=weights))
    x_var = float(np.average(np.square(x - x_mean), weights=weights))
    x_sd = math.sqrt(max(x_var, 0.0))
    if not math.isfinite(x_sd) or x_sd < 1e-8:
        return CalibrationFit(None, None, "NOT_ESTIMABLE", "near_constant_logit", n_clipped)
    z = (x - x_mean) / x_sd

    def objective_and_gradient(params: np.ndarray) -> tuple[float, np.ndarray]:
        eta = params[0] + params[1] * z
        probability = expit(np.clip(eta, -35.0, 35.0))
        probability = np.clip(probability, 1e-12, 1 - 1e-12)
        # ridge numérico desprezível para evitar Hessiana singular.
        ridge = 1e-10 * float(np.dot(params, params))
        nll = float(-np.sum(weights * (y * np.log(probability) + (1 - y) * np.log(1 - probability))) + ridge)
        residual = probability - y
        gradient = np.array([
            np.sum(weights * residual),
            np.sum(weights * residual * z),
        ], dtype=float) + 2e-10 * params
        return nll, gradient

    prevalence = float(np.average(y, weights=weights))
    starts = [
        np.array([0.0, 1.0]),
        np.array([float(logit(np.clip(prevalence, epsilon, 1 - epsilon))), 1.0]),
        np.array([0.0, 0.5]),
    ]
    best = None
    for start in starts:
        result = minimize(
            lambda params: objective_and_gradient(params)[0],
            start,
            jac=lambda params: objective_and_gradient(params)[1],
            method="L-BFGS-B",
            bounds=[(-30.0, 30.0), (-12.0, 12.0)],
            options={"maxiter": 5000, "ftol": 1e-12, "gtol": 1e-8},
        )
        if np.all(np.isfinite(result.x)) and math.isfinite(float(result.fun)):
            if best is None or float(result.fun) < float(best.fun):
                best = result
    if best is not None and (best.success or np.linalg.norm(objective_and_gradient(best.x)[1]) < 1e-4):
        alpha_z, beta_z = map(float, best.x)
        slope = beta_z / x_sd
        intercept = alpha_z - beta_z * x_mean / x_sd
        if math.isfinite(intercept) and math.isfinite(slope):
            return CalibrationFit(intercept, slope, "ESTIMATED", "weighted_likelihood_lbfgsb", n_clipped, float(best.fun))

    # Fallback: regressão logística quase não penalizada sobre logit padronizado.
    try:
        fallback = LogisticRegression(
            C=1e8, penalty="l2", solver="lbfgs", max_iter=5000,
            fit_intercept=True, random_state=20260724,
        )
        fallback.fit(z.reshape(-1, 1), y.astype(int), sample_weight=weights)
        alpha_z = float(fallback.intercept_[0])
        beta_z = float(fallback.coef_[0, 0])
        slope = beta_z / x_sd
        intercept = alpha_z - beta_z * x_mean / x_sd
        if math.isfinite(intercept) and math.isfinite(slope):
            return CalibrationFit(intercept, slope, "ESTIMATED_FALLBACK", "sklearn_near_unpenalized", n_clipped)
    except Exception:
        pass
    return CalibrationFit(None, None, "NOT_ESTIMABLE", "optimization_failed", n_clipped)
...
def evaluate_predictions(model_name: str, train_year: int, test_year: int, cells: pd.DataFrame, predictions: np.ndarray) -> MetricRow:
    ...
    calibration = calibration_intercept_slope(y, p, w)
    return MetricRow(
        model=model_name,
        ...
        calibration_n_clipped=calibration.n_clipped,
        probability_min=float(np.min(p)) if len(p) else None,
        probability_max=float(np.max(p)) if len(p) else None,
    )
...
def layout_equivalence(
    root: Path,
    upstream: Any,
    historical_lock: Mapping[str, Any],
    tests: list[TestResult],
    additional_roots: Sequence[str] | None = None,
) -> pd.DataFrame:
    ...
    for extra in additional_roots or []:
        candidate_root = Path(extra).expanduser()
        if not candidate_root.is_absolute():
            candidate_root = root / candidate_root
        search_roots.append(candidate_root.resolve())
    try:
    ...
    if len(independent_years) < 2:
        ...
        yearly = matrix[matrix["inferred_year"] != "unspecified"].copy()
        # Reconstrução explícita das assinaturas essenciais por variável.
        divergent = []
        for variable, group in yearly.groupby("variable", observed=True):
            sig = set(
                zip(
                    group["present"].astype(str), group["start_1based"].astype(str),
                    group["end_1based"].astype(str), group["width"].astype(str), group["type"].astype(str),
                )
            )
            if len(sig) > 1:
                divergent.append(str(variable))
        tests.append(TestResult(
            "layout.independent_signature_equivalence", "PASS" if not divergent else "WARN", "high",
            "Assinaturas essenciais equivalentes nas versões anuais independentes." if not divergent else "Divergências de assinatura entre versões anuais independentes; revisar antes da interpretação longitudinal.",
            observed=divergent, expected="sem divergências",
        ))
    return matrix
...
def bootstrap_model_uncertainty(
    ...
    if resume and checkpoint_path is not None and checkpoint_path.exists():
        try:
            ...
            if {"replicate", "source_period", "total", "share"}.issubset(existing.columns):
                completed = set(pd.to_numeric(existing["replicate"], errors="coerce").dropna().astype(int).tolist())
                logger.info("Retomando bootstrap: %d réplicas já completas", len(completed))
            else:
                existing = pd.DataFrame()
        except Exception as exc:
            logger.warning("Checkpoint de bootstrap não pôde ser lido: %s", exc)
            existing = pd.DataFrame()

    predictions: list[dict[str, Any]] = existing.to_dict("records") if not existing.empty else []
    failures: list[dict[str, Any]] = []
    for replicate in range(1, reps + 1):
        if replicate in completed:
            continue
        try:
            rng = np.random.default_rng(seed + replicate * 1_000_003)
            ...
            bundle = fit_mapping_bundle(boot, f"bootstrap_{replicate}", n_splits=n_splits, seed=seed + replicate)
            ...
            for period, group in merged.groupby("source_period", observed=True):
                ...
                predictions.append({
                    ...
                    "total": total,
                    "share": float(total / group["weight_total"].sum()),
                })
            completed.add(replicate)
            logger.info("Bootstrap de modelo %d/%d concluído", replicate, reps)
            if checkpoint_path is not None and (replicate % max(1, checkpoint_every) == 0 or replicate == reps):
                atomic_write_csv(checkpoint_path, pd.DataFrame(predictions).sort_values(["replicate", "source_period"]))
        except Exception as exc:
    ...
    if checkpoint_path is not None and not raw.empty:
        atomic_write_csv(checkpoint_path, raw.sort_values(["replicate", "source_period"]))
        failure_path = checkpoint_path.with_suffix(".failures.json")
        atomic_write_json(failure_path, failures)
    if raw.empty:
        return raw, pd.DataFrame(failures)
    raw = raw.drop_duplicates(["replicate", "source_period"], keep="last")
    ...
    summary["model_total_cv"] = summary["model_total_sd"] / summary["model_total_mean"].abs().replace(0, np.nan)
    summary["model_share_cv"] = summary["model_share_sd"] / summary["model_share_mean"].abs().replace(0, np.nan)
    summary.attrs["failures"] = failures
    return raw.sort_values(["replicate", "source_period"]), summary


def weighted_pearson_correlation(x: np.ndarray, y: np.ndarray, w: np.ndarray) -> float:
    ...
    vx = float(np.average(np.square(x - mx), weights=w))
    vy = float(np.average(np.square(y - my), weights=w))
    if vx <= 0 or vy <= 0:
        return np.nan
    return cov / math.sqrt(vx * vy)


def weighted_top_population_overlap(
    frame: pd.DataFrame,
    column_a: str,
    column_b: str,
    weight_column: str = "weight_total",
    top_share: float = 0.10,
) -> float:
    if frame.empty or frame[weight_column].sum() <= 0:
        return np.nan
    total = float(frame[weight_column].sum())
    cutoff = total * top_share

    def selected(column: str) -> set[int]:
        ordered = frame.sort_values(column, ascending=False).copy()
        ordered["_cum_weight"] = ordered[weight_column].cumsum()
        chosen = ordered[ordered["_cum_weight"] <= cutoff]
        if chosen.empty:
            chosen = ordered.iloc[:1]
        return set(chosen.index.tolist())

    set_a = selected(column_a)
    set_b = selected(column_b)
    union = set_a | set_b
    if not union:
        return np.nan
    intersection = set_a & set_b
    numerator = float(frame.loc[list(intersection), weight_column].sum()) if intersection else 0.0
    denominator = float(frame.loc[list(union), weight_column].sum())
    return numerator / denominator if denominator > 0 else np.nan


def aggregate_knn_stability(divergence: pd.DataFrame) -> pd.DataFrame:
    if divergence.empty:
        return pd.DataFrame()
    pooled = divergence["pooled_total"].to_numpy(dtype=float)
    knn = divergence["knn_total"].to_numpy(dtype=float)
    pearson = float(np.corrcoef(pooled, knn)[0, 1]) if len(divergence) >= 2 else np.nan
    try:
        rank = float(spearmanr(pooled, knn).statistic) if len(divergence) >= 2 else np.nan
    except Exception:
        rank = np.nan
    abs_diff = divergence["relative_total_difference"].abs()
    return pd.DataFrame([{
        "n_periods": int(len(divergence)),
        "temporal_pearson_total": pearson,
        "temporal_spearman_total": rank,
        "mean_absolute_relative_total_difference": float(abs_diff.mean()),
        "max_absolute_relative_total_difference": float(abs_diff.max()),
        "mean_micro_spearman": float(divergence["spearman_rank_correlation"].mean()),
        "mean_weighted_pearson_probability": float(divergence["weighted_pearson_probability"].mean()),
        "mean_top10_weighted_overlap": float(divergence["top10_weighted_overlap"].mean()),
    }])
...
def historical_predictions_and_estimates(
    ...
    for period, group in psu.groupby("source_period", observed=True):
        ...
        divergence_rows.append({
            ...
            "weighted_pearson_probability": weighted_pearson_correlation(
                feature_period["probability_mapping_pooled"].to_numpy(dtype=float),
                feature_period["probability_knn"].to_numpy(dtype=float),
                feature_period["weight_total"].to_numpy(dtype=float),
            ),
            "top10_weighted_overlap": weighted_top_population_overlap(
                feature_period,
                "probability_mapping_pooled",
                "probability_knn",
                top_share=0.10,
            ),
...
def test_metric_gates(metrics: pd.DataFrame, goldens: pd.DataFrame, tests: list[TestResult], min_auc: float, golden_max_relative_error: float) -> None:
    ...
    for row in temporal.itertuples(index=False):
        ...
        estimable = str(row.calibration_status).startswith("ESTIMATED") and pd.notna(row.calibration_slope)
        slope_ok = estimable and 0.75 <= row.calibration_slope <= 1.25
        if slope_ok:
            status, message = "PASS", "Slope de calibração estimado e em faixa pré-especificada."
        elif not estimable:
            status, message = "WARN", "Slope de calibração não estimável mesmo após estabilização numérica; usar Brier, ECE, reliability e goldens."
        else:
            status, message = "WARN", "Slope de calibração estimado fora de 0,75–1,25; interpretar probabilidades individuais com cautela."
        tests.append(TestResult(
            f"temporal.{row.train_year}_to_{row.test_year}.calibration_slope", status, "high",
            message,
            observed={"slope": row.calibration_slope, "intercept": row.calibration_intercept, "status": row.calibration_status, "method": row.calibration_method},
            expected="estimável e 0.75<=slope<=1.25",
        ))
...
{knn_aggregate.to_markdown(index=False) if not knn_aggregate.empty else 'Sem estabilidade agregada.'}
...
def audit_mode(root: Path, args: argparse.Namespace, logger: logging.Logger) -> int:
    ...
    layout_matrix = layout_equivalence(root, upstream, lock, tests, args.layout_root) if lock else pd.DataFrame()
    ...
    payload = {
        ...
        "upstream_lock_sha256": upstream_info.get("lock_sha256"),
        "upstream_freeze": upstream_info.get("freeze_path"),
        "upstream_freeze_sha256": upstream_info.get("freeze_sha256"),
        "layout_roots_additional": args.layout_root,
...
def full_mode(args: argparse.Namespace, logger: logging.Logger) -> int:
    ...
    layout_matrix = layout_equivalence(root, upstream, lock, tests, args.layout_root)
    layout_matrix.to_csv(paths["layout"], index=False)
    ...
    if tests_have_critical_failures(tests):
        status = "HARDENING_BLOCKED"
        report_text = build_report(run_id, status, tests, layout_matrix, pd.DataFrame(), pd.DataFrame(), pd.DataFrame(), pd.DataFrame(), pd.DataFrame(), pd.DataFrame(), pd.DataFrame(), args.profile)
    ...
    axis_columns = [column for column in support_table.columns if column.startswith("mca_axis_")]
    mca_cells_table = support_table[FEATURES + axis_columns].copy()
    support_columns = FEATURES + [
        "nearest_calibration_distance", "nearest_calibration_cell_index",
        "unseen_category_count", "exact_cell_2022", "exact_cell_2024", "support_class",
    ]
    support_export = support_table[support_columns].copy()
    mca_cells_table.to_csv(paths["mca_cells"], index=False)
    support_export.to_csv(paths["support"], index=False)
    ...
    knn_aggregate = aggregate_knn_stability(divergence)
    knn_aggregate.to_csv(paths["knn_aggregate"], index=False)
    ...
    # Diagnósticos KNN: o estimando é agregado; ranking micro é informativo, não gate principal.
    for row in divergence.itertuples(index=False):
        ...
        tests.append(TestResult(
            f"knn.{row.source_period}.micro_rank", "INFO", "secondary",
            "Concordância microindividual reportada apenas como diagnóstico; não identifica pessoas históricas.",
            observed={
                "spearman": row.spearman_rank_correlation,
                "weighted_pearson": row.weighted_pearson_probability,
                "top10_overlap": row.top10_weighted_overlap,
            },
            expected="informativo",
        ))
        tests.append(TestResult(
            f"knn.{row.source_period}.total", "PASS" if diff_ok else "WARN", "high",
            "Total KNN dentro da sensibilidade agregada prevista." if diff_ok else "Divergência agregada elevada entre logística e KNN.",
            observed=row.relative_total_difference, expected=f"abs(diff)<={args.knn_max_relative_difference}",
        ))
    if not knn_aggregate.empty:
        aggregate_row = knn_aggregate.iloc[0]
        correlation_ok = (
            pd.notna(aggregate_row["temporal_pearson_total"])
            and pd.notna(aggregate_row["temporal_spearman_total"])
            and aggregate_row["temporal_pearson_total"] >= args.knn_min_temporal_correlation
            and aggregate_row["temporal_spearman_total"] >= args.knn_min_temporal_correlation
        )
        difference_ok = pd.notna(aggregate_row["max_absolute_relative_total_difference"]) and aggregate_row["max_absolute_relative_total_difference"] <= args.knn_max_relative_difference
        tests.append(TestResult(
            "knn.aggregate.temporal_totals", "PASS" if correlation_ok and difference_ok else "WARN", "high",
            "Logística e KNN reproduzem de modo compatível o volume e a ordenação temporal agregada." if correlation_ok and difference_ok else "Robustez agregada logística/KNN abaixo do gate.",
            observed=aggregate_row.to_dict(),
            expected={"temporal_correlation": f">={args.knn_min_temporal_correlation}", "max_abs_total_diff": f"<={args.knn_max_relative_difference}"},
        ))
    ...
    bootstrap_raw, bootstrap_summary = bootstrap_model_uncertainty(
        ...
        n_splits=args.bootstrap_calibration_folds,
        logger=logger,
        checkpoint_path=paths["bootstrap_raw"],
        resume=args.resume_bootstrap,
        checkpoint_every=args.bootstrap_checkpoint_every,
        seed=args.bootstrap_seed,
    )
    if not bootstrap_raw.empty:
        atomic_write_csv(paths["bootstrap_raw"], bootstrap_raw)
    ...
    if args.bootstrap_reps > 0:
        minimum = max(1, math.ceil(args.bootstrap_reps * args.bootstrap_min_success_rate))
        bootstrap_ok = successful >= minimum
        severity = "critical" if args.profile == "publication" else "high"
        tests.append(TestResult(
            "bootstrap.model.successful_reps", "PASS" if bootstrap_ok else ("FAIL" if severity == "critical" else "WARN"), severity,
            "Réplicas de bootstrap suficientes para o perfil." if bootstrap_ok else "Número de réplicas bem-sucedidas insuficiente para o perfil solicitado.",
            observed={"successful": successful, "requested": args.bootstrap_reps, "rate": successful / args.bootstrap_reps if args.bootstrap_reps else np.nan},
            expected=f">={minimum}",
        ))
    ...
    hardening_bundle = {
        ...
        "bootstrap_reps_successful": int(successful),
        "features": FEATURES,
        "bundles": bundles,
        "mca": {
            "encoder": mca["encoder"],
    ...
    model_card = build_model_card(
        run_id, metrics_df, goldens_df, support_summary, divergence, knn_aggregate, mca,
        cluster_profiles_df, args.bootstrap_reps, args.profile,
    )
    atomic_write_text(paths["model_card"], model_card)
    report = build_report(
        run_id, status, tests, layout_matrix, metrics_df, goldens_df,
        final_estimates, support_summary, divergence, knn_aggregate, bootstrap_summary, args.profile,
    )
    ...
    registry_payload = {
        ...
        "upstream_historical_lock_sha256": upstream_info.get("lock_sha256"),
        "upstream_historical_freeze": upstream_info.get("freeze_path"),
        "upstream_historical_freeze_sha256": upstream_info.get("freeze_sha256"),
    ...
    lock_payload = {
        ...
        "historical_periods": sorted(historical_psu["source_period"].unique().tolist()),
        "execution_profile": args.profile,
        "certification_grade": "PUBLICATION" if args.profile == "publication" and successful >= 200 else "ENGINEERING",
        "bootstrap_reps_requested": int(args.bootstrap_reps),
        "bootstrap_reps_successful": int(successful),
    ...
    if status == "FINAL_CERTIFIED":
        ...
        final_lock = {
            ...
            "read_only": True,
            "execution_profile": args.profile,
            "certification_grade": lock_payload["certification_grade"],
            "bootstrap_reps_requested": int(args.bootstrap_reps),
            "bootstrap_reps_successful": int(successful),
            "evidence_tier": "C",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="PNADc Historical Backcast Publication Hardening & Uncertainty Engine")
    parser.add_argument("--root", required=True, help="Raiz do SPINE-GPEv7")
    parser.add_argument("--mode", choices=["audit", "full"], default="full")
    parser.add_argument("--profile", choices=["smoke", "production", "publication"], default="publication")
    parser.add_argument("--run-id", default=None, help="ID reusável para checkpoint/resume; omitido gera timestamp UTC.")
    parser.add_argument("--calibration-folds", type=int, default=None)
    parser.add_argument("--bootstrap-calibration-folds", type=int, default=None)
    ...
    parser.add_argument("--bootstrap-min-success-rate", type=float, default=0.95)
    ...
    parser.add_argument("--knn-min-rank-correlation", type=float, default=0.20, help="Diagnóstico micro; não é gate principal.")
    parser.add_argument("--knn-min-temporal-correlation", type=float, default=0.90)
    parser.add_argument("--knn-max-relative-difference", type=float, default=0.10)
    parser.add_argument("--max-out-of-support-expected-share", type=float, default=0.10)
    parser.add_argument("--layout-root", action="append", default=[], help="Raiz adicional para layouts anuais oficiais; pode ser repetida.")
    parser.add_argument("--strict", action="store_true", help="Gates críticos falham fechados.")
...
def apply_profile_defaults(args: argparse.Namespace) -> argparse.Namespace:
    defaults = {
        "smoke": {"calibration_folds": 3, "bootstrap_calibration_folds": 2, "bootstrap_reps": 10},
        "production": {"calibration_folds": 5, "bootstrap_calibration_folds": 3, "bootstrap_reps": 200},
        "publication": {"calibration_folds": 5, "bootstrap_calibration_folds": 3, "bootstrap_reps": 500},
    }[args.profile]
    for key, value in defaults.items():
        if getattr(args, key) is None:
            setattr(args, key, value)
    if args.bootstrap_reps < 0:
        raise ValueError("bootstrap_reps deve ser >=0")
    if not (0 < args.bootstrap_min_success_rate <= 1):
        raise ValueError("bootstrap_min_success_rate deve estar em (0,1].")
    return args
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    root = Path(args.root).expanduser().resolve()
    logger.info("SPINE-GPE PNADc Backcast Hardening v%s | root=%s | mode=%s | profile=%s", VERSION, root, args.mode, args.profile)
    if args.mode == "audit":
        return audit_mode(root, args, logger)
