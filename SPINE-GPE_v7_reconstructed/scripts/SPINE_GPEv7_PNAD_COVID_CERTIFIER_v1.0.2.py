...
# -*- coding: utf-8 -*-
"""
SPINE-GPE v7 — PNAD COVID Certification Engine v1.0.2
=======================================================
...
Exemplo Colab — setembro de 2020:
  python SPINE_GPEv7_PNAD_COVID_CERTIFIER_v1.0.2.py \
...
SEMANTIC_CONTRACT: dict[str, Any] = {
    ...
    "categorical_code_normalization": "integer-like strings canonicalized; 07, 7.0 and 7 map to 7",
...
OUTPUT_COLUMNS = [
    ...
    "C014", "social_security_response_valid", "social_security_contributor",
    "weekly_hours_usual", "weekly_hours_actual",
...
def canonical_category_code(series: pd.Series) -> pd.Series:
    """Canonicaliza categorias inteiras sem alterar identificadores alfanuméricos.

    Corrige a representação observada em alguns CSVs da PNAD COVID, nos quais
    categorias podem aparecer como 07, 7 ou 7.0. Valores inteiros são gravados
    sem zeros à esquerda; valores não inteiros são preservados após limpeza.
    """
    out = clean_code(series)
    numeric_mask = out.str.fullmatch(r"[+-]?\d+(?:\.0+)?", na=False)
    if numeric_mask.any():
        parsed = pd.to_numeric(out.loc[numeric_mask], errors="coerce").astype("Int64").astype("string")
        out.loc[numeric_mask] = parsed
    return out
...
def period_tag(months: Sequence[int]) -> str:
    months = sorted(set(int(m) for m in months))
    if len(months) == 1:
        return f"m{months[0]:02d}"
    return f"m{months[0]:02d}_m{months[-1]:02d}"
...
def find_dictionary(root: Path, month: int) -> Path | None:
    """Localiza o dicionário mensal sem construir padrões glob inválidos.

    A versão 1.0.0 concatenava ``*_`` com um sufixo que já continha ``*``
    (por exemplo, ``*_*.xls``), produzindo ``**`` dentro de um componente de
    caminho. Em Python 3.12 isso gera ``ValueError`` antes mesmo do download.

    A busca agora é:
      * recursiva via ``rglob``;
      * case-insensitive e tolerante a acentos;
      * limitada a arquivos ``.xls``/``.xlsx``;
      * deduplicada por caminho resolvido.
    """
    token = normalize_text(f"Dicionario_PNAD_COVID_{month:02d}2020")
    found: dict[str, Path] = {}
...
def transform_chunk(raw: pd.DataFrame, month: int, source_hash: str, offset: int) -> pd.DataFrame:
    ...
    codes = {name: canonical_category_code(col(name)) for name in ("C001", "C002", "C007", "C007B", "C007C", "C007D", "C014")}
    ...
    frame["pandemic_delivery_self_employed"] = nullable_bool(codes["C007"].eq(SEMANTIC_CONTRACT["self_employed_code"]) & delivery, delivery)
    ...
    frame["C014"] = codes["C014"]
    frame["social_security_response_valid"] = codes["C014"].notna().astype("boolean")
...
def write_month_to_writer(csv_path: Path, month: int, source_hash: str, writer: pq.ParquetWriter | None, output_path: Path, chunk_rows: int, logger: logging.Logger) -> tuple[pq.ParquetWriter, dict[str, Any]]:
    ...
    n_self_employed = n_c014_valid = n_c014_missing = 0
    observed_c007: set[str] = set()
    observed_c007c: set[str] = set()
    observed_c014: set[str] = set()
    ...
    for raw in pd.read_csv(csv_path, dtype="string", usecols=usecols, chunksize=chunk_rows, low_memory=False):
        ...
        observed_c007.update(transformed["C007"].dropna().astype(str).unique().tolist())
        observed_c007c.update(transformed["C007C"].dropna().astype(str).unique().tolist())
        observed_c014.update(transformed["C014"].dropna().astype(str).unique().tolist())
        if "V1013" in raw:
        ...
        delivery_mask = transformed["pandemic_delivery_observed"].fillna(False)
        n_self_employed += int(transformed.loc[delivery_mask, "pandemic_delivery_self_employed"].fillna(False).sum())
        n_c014_valid += int(transformed.loc[delivery_mask, "social_security_response_valid"].fillna(False).sum())
        n_c014_missing += int(delivery_mask.sum()) - int(transformed.loc[delivery_mask, "social_security_response_valid"].fillna(False).sum())
    ...
    return writer, {
        ...
        "goods_delivery": n_goods, "delivery_self_employed": n_self_employed,
        "delivery_C014_valid": n_c014_valid, "delivery_C014_missing": n_c014_missing,
        "observed_C007_codes": sorted(observed_c007),
        "observed_C007C_codes": sorted(observed_c007c),
        "observed_C014_codes": sorted(observed_c014),
...
def survey_mean(frame: pd.DataFrame, outcome: pd.Series, domain_indicator: pd.Series, month: int, domain: str, estimand: str, note_suffix: str = "") -> SurveyEstimate:
    valid = frame[["survey_weight", "survey_stratum", "survey_psu"]].copy()
    ...
    return SurveyEstimate(2020, month, domain, "Brasil", "BR", estimand, estimate, se, low, high, cv, len(valid), n_positive, n_valid, eff, n_psu, n_strata, df, precision_status(cv, eff, n_valid), f"Taylor linearization UPA/Estrato; singleton omitidos={singleton}; sem FPC.{note_suffix}")
...
def monthly_estimates(frame: pd.DataFrame, month: int) -> list[SurveyEstimate]:
    ...
    for col, estimand in (
        ...
        if estimand.startswith("percent"):
            outcome = outcome * 100
        suffix = ""
        if estimand == "percent_social_security":
            suffix = " Denominador: entregadores com resposta válida em C014; cobertura auditada separadamente."
        if estimand == "percent_self_employed":
            suffix = " C007 canonicalizada: representações 07, 7.0 e 7 são equivalentes ao código oficial 7."
        estimates.append(survey_mean(eligible, outcome, delivery, month, "pandemic_delivery_observed", estimand, suffix))
    return estimates


def monthly_coverage_audit(frame: pd.DataFrame, month: int) -> dict[str, Any]:
    delivery = frame["pandemic_delivery_observed"].fillna(False).astype(bool)
    subset = frame.loc[delivery].copy()
    weight = numeric(subset["survey_weight"]).fillna(0)
    c014_valid = subset["social_security_response_valid"].fillna(False).astype(bool)
    self_emp = subset["pandemic_delivery_self_employed"].fillna(False).astype(bool)
    total_weight = float(weight.sum())
    valid_weight = float(weight.loc[c014_valid].sum())
    return {
        "year": 2020,
        "month": month,
        "n_delivery": int(len(subset)),
        "n_self_employed": int(self_emp.sum()),
        "self_employed_percent_unweighted": float(self_emp.mean() * 100) if len(subset) else None,
        "n_C014_valid": int(c014_valid.sum()),
        "n_C014_missing": int((~c014_valid).sum()),
        "C014_coverage_percent_unweighted": float(c014_valid.mean() * 100) if len(subset) else None,
        "C014_coverage_percent_weighted": float(valid_weight / total_weight * 100) if total_weight > 0 else None,
        "C014_observed_codes": sorted(subset["C014"].dropna().astype(str).unique().tolist()),
        "denominator_rule": "percent_social_security usa somente entregadores com C014 observado",
    }
...
def load_month_frame(parquet_path: Path, month: int) -> pd.DataFrame:
    columns = [
        ...
        "pandemic_motoboy_observed", "pandemic_goods_delivery_observed",
        "C007", "C014", "pandemic_delivery_self_employed",
        "social_security_response_valid", "social_security_contributor",
...
def build_tests(month: int, frame: pd.DataFrame, source_manifest: Mapping[str, Any], dictionary: Mapping[str, Any], design: Mapping[str, Any]) -> list[TestResult]:
    ...
    expected_self = frame["C007"].eq(SEMANTIC_CONTRACT["self_employed_code"]) & delivery
    observed_self = frame["pandemic_delivery_self_employed"].fillna(False).astype(bool)
    self_derivation_ok = bool(observed_self.eq(expected_self.fillna(False).astype(bool)).all())
    tests.append(TestResult(
        f"2020m{month:02d}.self_employed.derivation",
        "PASS" if self_derivation_ok else "FAIL",
        "critical",
        "Conta-própria derivada de C007 canônica e restrita aos entregadores.",
        month,
        int(observed_self.sum()),
        int(expected_self.fillna(False).sum()),
        {"canonical_code": SEMANTIC_CONTRACT["self_employed_code"], "normalization": "07/7.0/7 -> 7"},
    ))
    self_positive = int(observed_self.loc[delivery].sum())
    tests.append(TestResult(
        f"2020m{month:02d}.self_employed.positive",
        "PASS" if self_positive > 0 else "FAIL",
        "high",
        "Há entregadores conta-própria observados após normalização de C007." if self_positive > 0 else "Nenhum entregador conta-própria foi identificado; revisar C007 e o dicionário.",
        month, self_positive, ">0",
        {"observed_C007_codes": sorted(frame.loc[delivery, "C007"].dropna().astype(str).unique().tolist())},
    ))

    c014_codes = set(frame.loc[delivery, "C014"].dropna().astype(str).unique().tolist())
    c014_invalid = sorted(c014_codes - {"1", "2"})
    tests.append(TestResult(
        f"2020m{month:02d}.domain.C014",
        "PASS" if not c014_invalid else "FAIL",
        "high",
        "C014 observado pertence ao domínio binário 1/2." if not c014_invalid else "C014 contém códigos não previstos no contrato binário.",
        month, sorted(c014_codes), ["1", "2"], {"invalid": c014_invalid},
    ))
    c014_valid = frame.loc[delivery, "social_security_response_valid"].fillna(False).astype(bool)
    coverage = float(c014_valid.mean() * 100) if positives else None
    coverage_status = "PASS" if coverage is not None and coverage >= 95 else "WARN"
    tests.append(TestResult(
        f"2020m{month:02d}.C014.coverage",
        coverage_status,
        "high",
        "Cobertura de C014 entre entregadores documentada; o percentual previdenciário usa somente respostas válidas.",
        month, coverage, ">=95% para cobertura ampla",
        {"n_delivery": positives, "n_valid": int(c014_valid.sum()), "n_missing": int((~c014_valid).sum())},
    ))
    tests.append(TestResult(
        f"2020m{month:02d}.C014.denominator_nonempty",
        "PASS" if int(c014_valid.sum()) >= 30 else "FAIL",
        "critical",
        "Denominador válido de C014 é suficiente para estimativa nacional." if int(c014_valid.sum()) >= 30 else "Menos de 30 entregadores com C014 válido.",
        month, int(c014_valid.sum()), ">=30",
    ))
    return tests
...
def render_report(
    ...
    status: str,
    months: list[int],
    source_manifests: Mapping[int, Any],
    freeze: Mapping[str, Any],
    designs: Mapping[int, Any],
    coverage_audits: Mapping[int, Any],
    estimates: list[SurveyEstimate],
    tests: list[TestResult],
    immutable_output: Path,
    latest_alias: Path,
) -> None:
    ...
    lines = [
        ...
        f"- Output imutável: `{immutable_output}`",
        f"- Alias latest: `{latest_alias}`",
        "",
        ...
        "## Fontes",
        "",
        "| Mês | Registros | Entrega | Motoboy | Entregador mercadorias | Conta-própria | C014 válido | C014 ausente | CSV | Dicionário |",
        "|---:|---:|---:|---:|---:|---:|---:|---:|---|---|",
    ]
    for month in months:
        manifest = source_manifests[month]
        lines.append(
            f"| {month:02d} | {manifest.get('rows')} | {manifest.get('pandemic_delivery_observed')} | "
            f"{manifest.get('motoboy')} | {manifest.get('goods_delivery')} | "
            f"{manifest.get('delivery_self_employed')} | {manifest.get('delivery_C014_valid')} | "
            f"{manifest.get('delivery_C014_missing')} | `{manifest.get('csv_path')}` | "
            f"`{manifest.get('dictionary_path')}` |"
        )
    ...
    lines += [
        "## Auditoria de derivados e denominadores",
        "",
        "| Mês | n entrega | n conta-própria | % conta-própria não ponderado | n C014 válido | n C014 ausente | Cobertura C014 não ponderada | Cobertura C014 ponderada | Domínio C014 |",
        "|---:|---:|---:|---:|---:|---:|---:|---:|---|",
    ]
    for month in months:
        audit = coverage_audits[month]
        lines.append(
            f"| {month:02d} | {audit.get('n_delivery')} | {audit.get('n_self_employed')} | "
            f"{fmt4(audit.get('self_employed_percent_unweighted'))} | "
            f"{audit.get('n_C014_valid')} | {audit.get('n_C014_missing')} | "
            f"{fmt4(audit.get('C014_coverage_percent_unweighted'))} | "
            f"{fmt4(audit.get('C014_coverage_percent_weighted'))} | "
            f"{','.join(audit.get('C014_observed_codes', []))} |"
        )
    lines += [
        "",
        "O percentual de contribuição previdenciária usa exclusivamente entregadores com resposta válida em `C014`; a cobertura e o missingness são reportados separadamente e não são recodificados.",
        "",
        "## Golden tests",
        "",
        "| Status | Severidade | Mês | Teste | Mensagem |",
        "|---|---|---:|---|---|",
    ]
    for test in tests:
    ...
    lines += [
        "",
        "## Estimativas mensais design-based",
        "",
        "| Mês | Domínio | Estimando | Estimativa | SE | CV% | n positivo | n outcome | n efetivo | Precisão | Nota |",
        "|---:|---|---|---:|---:|---:|---:|---:|---:|---|---|",
    ]
    for e in estimates:
        lines.append(
            f"| {e.month:02d} | {e.domain} | {e.estimand} | "
            f"{'' if e.estimate is None else f'{e.estimate:.4f}'} | "
            f"{'' if e.se is None else f'{e.se:.4f}'} | "
            f"{'' if e.cv_percent is None else f'{e.cv_percent:.2f}'} | "
            f"{e.n_positive} | {e.n_outcome_valid} | "
            f"{'' if e.n_effective_domain is None else f'{e.n_effective_domain:.1f}'} | "
            f"{e.precision_status} | {e.notes} |"
        )
    lines += [
        "", "## Regras congeladas", "",
        "1. C007C=16/17 mede ocupação de entrega no choque pandêmico, não plataforma direta.",
        "2. C007=7 significa conta própria; representações 07, 7.0 e 7 são canonicalizadas para 7.",
        ...
        "7. O denominador previdenciário é formado por entregadores com C014 válido; missingness é explicitado.",
        "8. Nenhuma localização fina ou plataforma foi imputada.",
        "9. O output canônico de cada execução é imutável; o arquivo sem Run ID é somente um alias latest.",
        "", "## Próximo gate", "",
...
def certify_mode(root: Path, months: list[int], download: bool, strict: bool, chunk_rows: int, cache_dir: Path, logger: logging.Logger) -> int:
    ...
    if any(t.status == "FAIL" and t.severity == "critical" for t in tests):
        ...
        lock = {
            ...
            "status": status,
            "critical_failures": [asdict(t) for t in tests if t.status == "FAIL"],
            "created_at_utc": utc_now(),
        }
    ...
    tag = period_tag(months)
    immutable_output = output_dir / f"certified_pnad_covid_delivery_2020_{tag}_{current_run}.parquet"
    staging_output = output_dir / f".{immutable_output.name}.partial"
    latest_alias = output_dir / "certified_pnad_covid_delivery_2020.parquet"
    ...
    try:
        for month in months:
            ...
            writer, summary = write_month_to_writer(csv_path, month, csv_hash, writer, staging_output, chunk_rows, logger)
            summary.update({
            ...
            layout_months[str(month)] = {
                ...
                "required_columns": RAW_KEEP,
                "required_present": sorted(set(RAW_KEEP) & set(header)),
    ...
    if writer is None or not staging_output.exists():
        tests.append(TestResult("certified_table.created", "FAIL", "critical", "Parquet certificado não foi criado."))
    else:
        tests.append(TestResult("certified_table.created", "PASS", "critical", "Parquet candidato criado em staging imutável."))

    designs: dict[int, Any] = {}
    coverage_audits: dict[int, Any] = {}
    estimates: list[SurveyEstimate] = []
    if staging_output.exists():
        for month in months:
            if month not in source_manifests:
                continue
            frame = load_month_frame(staging_output, month)
            design = design_summary(frame, month)
            designs[month] = design
            coverage_audits[month] = monthly_coverage_audit(frame, month)
        ...
        months_in_output = sorted(
            pd.read_parquet(staging_output, columns=["reference_month"])["reference_month"]
            .dropna().astype(int).unique().tolist()
        )
        tests.append(TestResult(
            "period.months_preserved",
            "PASS" if months_in_output == months else "FAIL",
            "critical",
            "Meses solicitados preservados no Parquet." if months_in_output == months else "Meses do output divergem dos solicitados.",
            observed=months_in_output,
            expected=months,
        ))
        direct_values = pd.read_parquet(staging_output, columns=["platform_delivery_direct"])["platform_delivery_direct"]
        tests.append(TestResult(
            "global.no_proxy_platform_fill",
            "PASS" if direct_values.isna().all() else "FAIL",
            "critical",
            "Nenhum registro recebeu identificação direta de plataforma por proxy.",
        ))
    ...
    dictionary_all = bool(source_manifests) and all(
        source_manifests[m]["dictionary_evidence"].get("contains_code_16_motoboy")
        and source_manifests[m]["dictionary_evidence"].get("contains_code_17_delivery")
        for m in source_manifests
    )
    if critical_failures:
        status = "BLOCKED"
    elif secondary_failures or not dictionary_all:
    ...
    if staging_output.exists():
        if critical_failures:
            final_output = output_dir / f"certified_pnad_covid_delivery_2020_{tag}_{current_run}_BLOCKED.parquet"
            os.replace(staging_output, final_output)
        else:
            os.replace(staging_output, immutable_output)
            final_output = immutable_output
            atomic_copy(final_output, latest_alias)
    else:
        final_output = immutable_output

    registry = root / "00_admin" / "registry"
    registry.mkdir(parents=True, exist_ok=True)
    layout_stamped = registry / f"pnad_covid_layout_2020_{current_run}.json"
    design_stamped = registry / f"survey_design_pnad_covid_2020_{current_run}.json"
    golden_stamped = registry / f"golden_tests_pnad_covid_2020_{current_run}.json"
    manifest_stamped = registry / f"certified_pnad_covid_delivery_2020_manifest_{current_run}.json"
    layout_latest = registry / "pnad_covid_layout_2020.json"
    design_latest = registry / "survey_design_pnad_covid_2020.json"
    golden_latest = registry / "golden_tests_pnad_covid_2020.json"
    manifest_latest = registry / "certified_pnad_covid_delivery_2020_manifest.json"

    layout_payload = {
        "schema_version": SCHEMA_VERSION,
        "created_at_utc": utc_now(),
        "semantic_contract": SEMANTIC_CONTRACT,
        "months": layout_months,
    }
    design_payload = {
        ...
        "panel_note": "Amostra fixa/repetida; estimativas e variâncias certificadas por mês.",
    }
    golden_payload = {
        "schema_version": VALIDATION_SCHEMA_VERSION,
        "status": status,
    ...
    output_hash = sha256_file(final_output) if final_output.exists() else None
    output_rows = int(pq.ParquetFile(final_output).metadata.num_rows) if final_output.exists() else None
    manifest = {
        "run_id": current_run,
        "script_version": VERSION,
        "schema_version": SCHEMA_VERSION,
        ...
        "immutable_output": str(final_output),
        "latest_alias": str(latest_alias) if not critical_failures else None,
        "output_sha256": output_hash,
        "rows": output_rows,
        "sources": source_manifests,
        "semantic_contract": SEMANTIC_CONTRACT,
        "coverage_audits": coverage_audits,
        "upstream_freeze": freeze,
        "no_proxy_fill": True,
        "synthetic_location": False,
        "platform_direct_available": False,
        "created_at_utc": utc_now(),
    }
    for stamped, latest, payload in (
        (layout_stamped, layout_latest, layout_payload),
        (design_stamped, design_latest, design_payload),
        (golden_stamped, golden_latest, golden_payload),
        (manifest_stamped, manifest_latest, manifest),
    ):
        json_dump(stamped, payload)
        atomic_copy(stamped, latest)

    estimates_dir = root / "05_outputs" / "tables" / "pnad_covid"
    estimates_dir.mkdir(parents=True, exist_ok=True)
    estimates_stamped = estimates_dir / f"pnad_covid_delivery_estimates_2020_{current_run}.csv"
    estimates_latest = estimates_dir / "pnad_covid_delivery_estimates_2020.csv"
    pd.DataFrame([asdict(e) for e in estimates]).to_csv(estimates_stamped, index=False, encoding="utf-8-sig")
    atomic_copy(estimates_stamped, estimates_latest)

    report_dir = root / "06_reports" / "pnad_covid_certification"
    report_dir.mkdir(parents=True, exist_ok=True)
    report_stamped = report_dir / f"pnad_covid_certification_report_{current_run}.md"
    report_latest = report_dir / "pnad_covid_certification_report.md"
    render_report(
        report_stamped,
        current_run,
        status,
    ...
    atomic_copy(report_stamped, report_latest)

    artifact_hashes = {
        "output": output_hash,
        "report": sha256_file(report_stamped),
        "manifest": sha256_file(manifest_stamped),
        "layout": sha256_file(layout_stamped),
        "survey_design": sha256_file(design_stamped),
        "golden_tests": sha256_file(golden_stamped),
        "estimates": sha256_file(estimates_stamped),
    }
    lock = {
        "run_id": current_run,
        "script_version": VERSION,
        ...
        "warnings": warnings,
        "report": str(report_stamped),
        "report_latest": str(report_latest),
        "output": str(final_output),
        "output_latest": str(latest_alias) if not critical_failures else None,
        "manifest": str(manifest_stamped),
        "manifest_latest": str(manifest_latest),
        "layout": str(layout_stamped),
        "survey_design": str(design_stamped),
        "golden_tests": str(golden_stamped),
        "estimates": str(estimates_stamped),
        "coverage_audits": coverage_audits,
        "artifact_hashes": artifact_hashes,
        "pnadc_core_freeze": freeze.get("path"),
        "created_at_utc": utc_now(),
    }
    lock_path = root / "00_admin" / "PNAD_COVID_CERTIFICATION_LOCK.json"
    json_dump(lock_path, lock)
    logger.info("Certificação PNAD COVID concluída | status=%s | relatório=%s | output=%s", status, report_stamped, final_output)
