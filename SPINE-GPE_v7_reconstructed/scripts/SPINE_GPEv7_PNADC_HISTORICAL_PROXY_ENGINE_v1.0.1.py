...
---------
Auditoria:
  python SPINE_GPEv7_PNADC_HISTORICAL_PROXY_ENGINE_v1.0.1.py \
...
SCRIPT_NAME = "SPINE_GPEv7_PNADC_HISTORICAL_PROXY_ENGINE_v1.0.1.py"
UTC = dt.timezone.utc
...
HISTORICAL_LAYOUT_REQUIRED = {"V4010", "V4013", "VD4009"}
HISTORICAL_LAYOUT_SURVEY = {"V1028", "ESTRATO", "UPA"}
DIRECT_LAYOUT_ONLY = {"S140093", "SD14001"}


def choose_layout_for_source(
    upstream: Any,
    source: Path,
    period: Period,
    layouts: Sequence[Any],
) -> tuple[Any | None, int | None]:
    """Seleciona layout *regular* para TXT histórico.

    O seletor ``upstream.select_layout`` pertence ao certifier dos suplementos
    diretos e, por contrato, exige S140093. A PNADc trimestral regular deve usar
    a política inversa: largura exata, variáveis históricas essenciais e ausência
    das variáveis exclusivas do módulo de plataformas.
    """
    ...
    exact_width_candidates: list[dict[str, Any]] = []

    for candidate in layouts:
        names = {str(field.variable).upper() for field in candidate.fields}
        if candidate.width == width:
            exact_width_candidates.append({
                "path": candidate.path,
                "width": candidate.width,
                "has_S140093": bool(getattr(candidate, "has_s140093", False)),
                "has_SD14001": "SD14001" in names,
                "historical_required_present": sorted(HISTORICAL_LAYOUT_REQUIRED & names),
                "historical_required_missing": sorted(HISTORICAL_LAYOUT_REQUIRED - names),
            })

        # Histórico regular: largura deve casar exatamente e nenhuma variável
        # exclusiva do suplemento direto pode estar presente.
        if candidate.width != width:
            continue
        if bool(getattr(candidate, "has_s140093", False)) or (DIRECT_LAYOUT_ONLY & names):
            continue

        required_count = len(HISTORICAL_LAYOUT_REQUIRED & names)
        survey_count = len(HISTORICAL_LAYOUT_SURVEY & names)
        score = int(getattr(candidate, "year_score", {}).get(str(period.year), 0))
        score += 1_000
        score += 100 * required_count
        score += 20 * survey_count

        path_lower = str(candidate.path).lower()
        if "dicionario" in Path(candidate.path).name.lower():
            score += 5
        if "platform" in path_lower or "plataforma" in path_lower:
            score -= 500

        scored.append((score, len(candidate.fields), str(candidate.path), candidate))

    if not scored:
        summary = sorted(
            exact_width_candidates,
            key=lambda row: (
                len(row["historical_required_missing"]),
                row["has_S140093"],
                row["path"],
            ),
        )[:12]
        raise RuntimeError(
            f"Nenhum layout regular casou com a largura {width} para {period.year}. "
            f"Candidatos de largura exata: {summary}"
        )

    # Maior score e maior cobertura de campos; caminho lexical resolve empates
    # de forma determinística entre cópias equivalentes do mesmo dicionário.
    scored.sort(key=lambda item: (-item[0], -item[1], item[2]))
    _, _, _, layout = scored[0]
    variables = {str(field.variable).upper() for field in layout.fields}
    missing = sorted(HISTORICAL_LAYOUT_REQUIRED - variables)
    if missing:
        summary = [{
            "score": score,
            "path": candidate.path,
            "width": candidate.width,
            "missing": sorted(HISTORICAL_LAYOUT_REQUIRED - {
                str(field.variable).upper() for field in candidate.fields
            }),
        } for score, _, _, candidate in scored[:10]]
        raise RuntimeError(
            f"Layouts regulares com largura {width} não contêm todas as variáveis "
            f"históricas essenciais {sorted(HISTORICAL_LAYOUT_REQUIRED)}. "
            f"Melhores candidatos: {summary}"
        )
...
def audit_mode(root: Path, periods_spec: str, download_missing: bool, strict: bool, logger: logging.Logger) -> int:
    ...
    for period in requested:
        ...
        try:
            ...
            regular_layout_ok = not bool(DIRECT_LAYOUT_ONLY & upper_columns)
            tests.append(TestResult(
                f"historical.{period.key}.layout_regular",
                "PASS" if regular_layout_ok else "FAIL",
                "critical",
                "Layout trimestral regular selecionado; variáveis diretas ausentes."
                if regular_layout_ok else
                "Layout histórico contém variável exclusiva do módulo direto.",
                period=period.key,
                observed={
                    "layout": getattr(layout, "path", None) if layout else None,
                    "record_width": width,
                    "direct_variables_present": sorted(DIRECT_LAYOUT_ONLY & upper_columns),
                },
            ))
            required = {"V4010", "V4013", "VD4009"}
